...
const fallbackProjects: Project[] = [
  {
    ...
    description:
      'A highly interactive personal portfolio built from scratch with React, Three.js and Framer Motion physics animations and advanced CSS blending techniques.',
...
const ProjectCard = ({ project, index }: { project: Project; index: number }) => (
  <article className="card-hard group flex flex-col p-7 hover:bg-paper-2 transition-colors duration-200">
    ...
    <p className="font-body text-[15px] text-ink-2 leading-relaxed mb-6">{project.description}</p>

    <div className="mt-auto flex flex-wrap gap-2 mb-6">
      {project.topics?.slice(0, 4).map((t, i) => (
        <span
          key={i}
          className="font-mono text-[11px] uppercase tracking-[0.06em] px-2.5 py-1 border border-hairline text-ink-2"
        >
          {t}
        </span>
      ))}
    ...
    <div className="flex gap-5 pt-5 border-t-2 border-ink">
      {project.homepage && (
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="flex items-center gap-1.5 font-mono text-xs uppercase tracking-[0.08em] text-ink hover:text-accent transition-colors"
        >
          ...
          const formatted: Project[] = data
            ...
            .map((repo) => {
              ...
              return {
                ...
                description:
                  repo.description ||
                  `An open-source repository focused on ${repo.language || 'software'} development and implementation.`,
                topics:
    ...
    >
      <header className="mb-12 max-w-3xl">
        ...
        <p className="mt-5 font-body text-lg text-ink-2 leading-relaxed">
          Technical projects bridging complex theoretical modeling with practical hardware and
          software deployments. Pulled live from GitHub.
        </p>
      </header>
