        ...
        {GROUPS.map((g) => (
          <div key={g.code} className="card-hard">
            <div className="flex items-center justify-between px-5 py-4 border-b-2 border-ink bg-paper-2">
              <h3 className="font-display font-extrabold text-xl uppercase tracking-tight text-ink">
                {g.title}
              </h3>
            ...
            <dl>
              {g.rows.map((r) => (
                ...
                >
                  ...
                  <dd
                    className={`font-mono text-xs uppercase tracking-[0.1em] shrink-0 ${
                      r.level === 'Advanced' ? 'text-accent' : 'text-ink-2'
                    }`}
                  >
                    {r.level}
                  </dd>
