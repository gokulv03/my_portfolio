    ...
    <section id="top" className="relative pt-12 sm:pt-16">
      <div className="mx-auto w-full max-w-[1400px] px-5 sm:px-8 lg:px-12">
        ...
        <motion.div initial="hidden" animate="visible" variants={container}>
          ...
          <motion.div
            variants={item}
            className="mt-8 sm:mt-10 grid lg:grid-cols-12 gap-6 lg:gap-10 items-end"
          >
            <p className="lg:col-span-7 font-body text-lg sm:text-xl text-ink-2 leading-relaxed max-w-2xl">
              M.Sc. Electrical Engineering student specializing in Automation Technology. I
              bridge theoretical modeling with robust hardware deployments across{' '}
          ...
          >
            <a
              href="mailto:gokulv0331@gmail.com"
              className="group flex items-center gap-3 py-4 sm:pr-6 border-b sm:border-b-0 sm:border-r border-hairline font-mono text-sm text-ink hover:text-accent transition-colors"
            >
            ...
            <a
              href="https://www.linkedin.com/in/gokul-vemulapalli"
              target="_blank"
              rel="noreferrer"
              className="flex items-center justify-between gap-2 py-4 sm:px-6 border-b sm:border-b-0 sm:border-r border-hairline font-mono text-sm text-ink hover:text-accent transition-colors"
            >
              LinkedIn <ArrowUpRight className="w-4 h-4 shrink-0" />
            </a>
            <a
              href="https://github.com/gokulv03"
              target="_blank"
              rel="noreferrer"
              className="flex items-center justify-between gap-2 py-4 sm:pl-6 font-mono text-sm text-ink hover:text-accent transition-colors"
            >
          ...
          {[...Array(2)].map((_, loop) => (
            <div key={loop} className="flex shrink-0 items-center">
              {TECH.map((t, i) => (
                <span
                  key={i}
                  className="flex items-center font-mono font-bold uppercase tracking-[0.1em] text-ink-2 text-base sm:text-xl px-6 sm:px-10"
                >
                  {t}
                  <span className="ml-12 sm:ml-20 text-accent" aria-hidden="true">
                    /
                  </span>
                </span>
              ))}
